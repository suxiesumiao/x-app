		...
		>
			...
			>
				<view
					...
					v-if="mode == 'number'"
					:style="{
						backgroundColor: current < index ? 'transparent' : (current === index? viceActiveColor: activeColor),
						borderColor: current < index ? unActiveColor : (current === index? viceActiveColor: activeColor)
					}"
				>
          ...
          <u-icon v-else-if="current === index" size="22" color="#ffffff" :name="viceicon || icon"></u-icon>
				...
				<text :style="{ 
					color: index < current ? activeColor : (index === current? viceActiveColor: unActiveColor),
