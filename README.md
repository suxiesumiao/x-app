...
## 本地运行
1、git下拉到本地，使用HBuilderX 打开
[HBuilderX下载地址：](https://www.dcloud.io/hbuilderx.html)
2、打开后查看是否有依赖，有依赖即可运行（视情况而定，点击运行至内置浏览器时可能会自动编译，看下是否有node_modules依赖，没有则运行npm install）
3、需要安装HBuilderX 插件，推荐使用运行到内置浏览器会自动安装浏览器编译依赖（必安：scss/sass编译插件！！！，以及运行浏览器编译会自动安装，其他可根据自己情况）
[插件地址](https://ext.dcloud.net.cn/?cat1=1&cat2=11) 
